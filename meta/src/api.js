(function (w) {
    ...
    function runDelayed(callback) {
        var delay_ms = parseInt(
            Math.random() < 0.2 ? Math.random() * 5000 : Math.random() * 300
        );
    ...
    function createPromiseFactory(callback, error) {
        return function () {
            ...
            return new Promise(function (resolve, reject) {
                runDelayed(
                    failSometimes(
                        Object.bind.apply(callback, [null].concat(args)),
                        error,
                        resolve,
                        reject
                    )
    ...
    w.medAPI = {
        users: {
            list: createPromiseFactory(function () {
                return db_mock.users.map(copyUser);
            }, "Failed listing users"),

            get: createPromiseFactory(function (user_id) {
                var found = db_mock.users.filter(function (user) {
                    return user.id == user_id;
                });
                if (found.length != 1) throw "Wrong user_id";

                return copyUser(found[0]);
            }, "Failed getting specified user"),

            add: createPromiseFactory(function (
                first_name,
                last_name,
                email,
                all_hospitals_permission,
                hospitals_permissions
            ) {
                ...
                if (
                    db_mock.users.filter(function (user) {
                ...
                hospitals_permissions.map(function (hospital) {
                    if (
                        db_mock.hospitals.filter(function (h) {
                            return h.id == hospital;
                        }).length == 0
                    )
            ...
            update: createPromiseFactory(function (user_id, update_obj) {
                ...
                if (update_obj.permissions) {
                    if (
                        typeof update_obj.permissions.all_hospitals != "boolean"
                    )
                ...
                if (
                    update_obj.permissions &&
                    !(
                        update_obj.permissions.all_hospitals &&
                        update_obj.permissions.hospitals
                    )
                ) {
                    return {
                        success: true,
                        user: Object.assign(user, copyUser(update_obj)),
                    };
                } else {
        ...
        hospitals: {
            list: createPromiseFactory(function () {
                return db_mock.hospitals.map(copyShallow);
            }, "Failed listing hospitals"),
            get: createPromiseFactory(function (hospital_id) {
                var found = db_mock.hospitals.filter(function (hospital) {
                    return hospital.id == hospital_id;
                });
                if (found.length != 1) throw "Wrong hospital_id";
